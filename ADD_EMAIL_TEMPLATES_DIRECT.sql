-- DIREKT IN RAILWAY POSTGRESQL AUSFÜHREN!
-- Gehen Sie zu Railway → PostgreSQL → Query Tab
-- Kopieren Sie dieses SQL und führen Sie es aus

-- 1. Spalte fixen (Default entfernen, dann konvertieren)
ALTER TABLE email_templates ALTER COLUMN variables DROP DEFAULT;
ALTER TABLE email_templates ALTER COLUMN variables TYPE TEXT[] USING CASE 
  WHEN variables IS NULL THEN '{}'::text[]
  ELSE ARRAY(SELECT jsonb_array_elements_text(variables))
END;

-- 2. Neue Templates hinzufügen
